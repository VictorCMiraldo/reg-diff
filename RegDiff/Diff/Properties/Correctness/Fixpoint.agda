...
-}
  ...
  mutual
    ...
    Dμ-ins-src
      ...
      ≡ x
    Dμ-ins-src {O} {ty} x y hip = aux (μ-chv y) hip
      where
        aux : {k : ℕ}(v : Vec (μ ty) k)(h : ¬ k ≡ 0) → Dμ-src (p2 (do-ins x v h)) ≡ x
        aux [] h = ⊥-elim (h refl)
        aux (v ∷ vs) h = diffμ-src-lemma x (lookup (O x (v ∷ vs)) (v ∷ vs))
  ...
  mutual
    ...
    Dμ-del-dst
      ...
      ≡ y
    Dμ-del-dst {O} {ty} x y hip = aux (μ-chv x) hip
      where
        aux : {k : ℕ}(v : Vec (μ ty) k)(h : ¬ k ≡ 0) → Dμ-dst (p2 (do-del v h y)) ≡ y
        aux [] h = ⊥-elim (h refl)
        aux (v ∷ vs) h = diffμ-dst-lemma {O} (lookup (O y (v ∷ vs)) (v ∷ vs)) y
