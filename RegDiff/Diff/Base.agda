...
-}
  ...
  ty-diffs : (k : Fin n) → Dₐ (lookup k v)
  ty-diffs k = lookupᵢ k tds
...
-}
  ...
  D-src D1 = unit
  D-src (DA x y) = x
  D-src (DK k x) = srcₐ (ty-diffs k) x
  ...
  D-dst D1 = unit
  D-dst (DA x y) = y
  D-dst (DK k x) = dstₐ (ty-diffs k) x
...
-}
  ...
  cost D1 = 1
  cost (DA x y) = 1
  cost (DK k x) = costₐ (ty-diffs k) x
