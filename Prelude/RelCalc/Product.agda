...
module Prelude.RelCalc.Product where
  ...
  abstract
    ...
    prod-uni-r1 : ∀{a}{A B C : Set a}{X : (A × B) ⟵ C}
               ...
               → X ⊆ < R ∣ S >
               → π₁ ∙ X ⊆ R
    prod-uni-r1 {X = X} R S hip {.a} {c} ((a , b) , cons-fun refl , h1)
      = p1 (<_∣_>.un (hip h1))
    ...
    prod-uni-r2 : ∀{a}{A B C : Set a}{X : (A × B) ⟵ C}
               ...
               → X ⊆ < R ∣ S >
               → π₂ ∙ X ⊆ S
    prod-uni-r2 {X = X} R S hip {.b} {c} ((a , b) , cons-fun refl , h1)
      = p2 (<_∣_>.un (hip h1))
    ...
    prod-uni-l {X = X} R S pr ps {a , b} {c} hip
      = cons-split ((pr ((a , b) , (cons-fun refl , hip))) 
                   , ps ((a , b) , (cons-fun refl , hip)))
